...
Because the compilation of these executable can be difficult (due to library dependencies) I have also made the precompiled executables available [here](https://drive.google.com/open?id=1_YUNYtVJ0aP7iYplnDaT-UcJnd_cx4E-). To 'install' these executables for use in SymPortal, simply copy them into the [./lib/phylipnew/](https://github.com/didillysquat/SymPortal_framework/tree/master/lib/phylipnew) directory. SymPortal will automatically check to see whether the required executables are found in your system's PATH. If not, it will search for them in this ./lib/phylipnew/ directory.
***
